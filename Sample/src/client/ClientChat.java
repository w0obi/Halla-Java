...
public class ClientChat extends JFrame implements ActionListener {
     ...
     * 코맨트, 이름, gui변경
     * 
     * 2주 동안 결과는 ppt 10장 이내, 작업 사진 1~2장, 결과 소스 파일
     */
    private void sendNote() {
        System.out.println("noteBtn clicked");
        String dstClient = (String) clientList.getSelectedValue();   // 쪽지 수신

        String note = JOptionPane.showInputDialog("보낼 쪽지");
        if(note != null) {
            sendMsg("Note/" + dstClient + "/" + note);
            System.out.println("receiver : " + dstClient + "| send note : " + note);
        }
    }
